...
def runge_kutta(init_conditions, time, num_timesteps, eq_matrix):
    """
    Function Description:
        - Solves a given differential equation using Runge-Kutta Method
        - This method uses an average of the slope at the beginning, end, and along the interval
    ...
    for i in range(1, len(t)):
        k_n1 = np.dot(eq_matrix, solution[:, i-1])
        k_n2_predict = solution[:, i - 1] + (dt / 2) * 

        # # Step 1: Predictor (Compute the slopes of each k along the interval)
