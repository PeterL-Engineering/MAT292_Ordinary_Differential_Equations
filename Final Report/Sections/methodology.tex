...
In our project, we implemented three numerical methods for solving the Hodgkin-Huxley differential equations: Euler's method, improved Euler's method, and the fourth-order Runge-Kutta method.  
...
These methods were adapted from EEG noise models but scaled to action potential amplitudes.  
\textbf{CITE PAPER WE FOUND THAT MODELED EEG NOISE}
...
The implemented function uses a circular buffer approach to efficiently compute this convolution.  
...
The output at each step is computed as the dot product of the buffer and coefficient vector.  
This implementation minimizes memory operations while mathematically equivalent to direct convolution.
