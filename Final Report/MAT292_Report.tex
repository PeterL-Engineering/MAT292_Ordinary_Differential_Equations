...
\begin{itemize}[leftmargin=*,noitemsep]
    \item \textbf{Week 4:} Complete Euler's and Improved Euler's method solvers and first noise generation algorithm.
    \item \textbf{Week 6:} Implement Runge-Kutta method solver(s) and first band-pass filter iteration.
    \item \textbf{Week 8:} Refine all components: band-pass filter, noise generation, and numerical methods.
    \item \textbf{Week 10:} Final results evaluation and report completion.
    \item \textbf{Weeks 11-12:} Buffer time for unexpected delays.
\end{itemize}
...
In this section, we outline Euler's method which forms the foundation for the latter two methods. 
...
\section{Numerical Method Accuracy}

The Local Truncation Error (LTE) measures the error committed in a single time step of a numerical method for solving ordinary differential equations. For a general ODE system:
\begin{equation}
\frac{dy}{dt} = f(t, y), \quad y(t_0) = y_0
\end{equation}
the LTE at step $n$ is defined as:
\begin{equation}
\tau_n = \frac{y(t_{n+1}) - y_{n+1}}{h}
\end{equation}
where $y(t_{n+1})$ is the exact solution at $t_{n+1}$, and $y_{n+1}$ is the numerical approximation obtained by applying the method starting from the exact value $y(t_n)$.

The general procedure for LTE analysis involves:
\begin{enumerate}
    \item Taylor expand the exact solution $y(t_{n+1})$ around $t_n$
    \item Express the numerical method $y_{n+1}$ in terms of $y(t_n)$ and $f$
    \item Expand $f$ evaluations using multivariate Taylor series when necessary
    \item Compare the two expressions term by term
    \item Identify the leading error term
\end{enumerate}

For multivariate problems like the Hodgkin-Huxley equations, we employ Jacobian matrices in the Taylor expansions. The Jacobian $J_f$ of $f(t,y)$ is defined as:
\begin{equation}
J_f = \frac{\partial f}{\partial y} = \begin{bmatrix}
...
Starting from the exact solution $y(t_n)$, the numerical approximation is:
\begin{equation}
y_{n+1} = y(t_n) + h f(t_n, y(t_n))
\end{equation}

The exact solution Taylor expands as:
\begin{equation}
y(t_{n+1}) = y(t_n) + h y'(t_n) + \frac{h^2}{2} y''(t_n) + \frac{h^3}{6} y'''(t_n) + O(h^4)
\end{equation}

Since $y'(t_n) = f(t_n, y(t_n))$, we compute higher derivatives using the chain rule:
\begin{equation}
y''(t_n) = \frac{d}{dt}f(t,y(t))\big|_{t=t_n} = f_t(t_n, y(t_n)) + J_f(t_n, y(t_n)) f(t_n, y(t_n))
\end{equation}
where $f_t = \frac{\partial f}{\partial t}$ and $J_f$ is the Jacobian matrix.

The LTE becomes:
\begin{align}
\tau_n &= \frac{1}{h} \left[ y(t_{n+1}) - y_{n+1} \right] \\
&= \frac{1}{h} \left[ \left( y(t_n) + h f + \frac{h^2}{2} (f_t + J_f f) + O(h^3) \right) - \left( y(t_n) + h f \right) \right] \\
&= \frac{h}{2} (f_t + J_f f) + O(h^2)
\end{align}

Thus, Euler's method has LTE of order $O(h)$:
\begin{equation}
\tau_n = \frac{h}{2} y''(t_n) + O(h^2) = O(h)
\end{equation}

\subsection{Improved Euler's Method}

The Improved Euler (Heun's) method is:
\begin{align}
k_1 &= f(t_n, y_n) \\
...
Starting from exact values:
\begin{equation}
y_{n+1} = y(t_n) + \frac{h}{2} \left[ f(t_n, y(t_n)) + f(t_n + h, y(t_n) + h f(t_n, y(t_n))) \right]
\end{equation}

Expand $f(t_n + h, y(t_n) + h f)$ using multivariate Taylor series:
\begin{align}
f(t_n + h, & y + h f) = f(t_n, y) + h f_t(t_n, y) + h J_f(t_n, y) f(t_n, y) \\
& + \frac{h^2}{2} \left[ f_{tt}(t_n, y) + 2 f_t(t_n, y) J_f(t_n, y) f(t_n, y) + f(t_n, y)^\top H_f(t_n, y) f(t_n, y) \right] + O(h^3)
\end{align}
where $H_f$ is the Hessian tensor of second partial derivatives.

The numerical method becomes:
\begin{align}
y_{n+1} &= y(t_n) + \frac{h}{2} \left[ 2f + h(f_t + J_f f) + \frac{h^2}{2}(f_{tt} + 2f_t J_f f + f^\top H_f f) + O(h^3) \right] \\
&= y(t_n) + h f + \frac{h^2}{2} (f_t + J_f f) + \frac{h^3}{4} (f_{tt} + 2f_t J_f f + f^\top H_f f) + O(h^4)
\end{align}

The exact solution expansion up to third order is:
\begin{align}
y(t_{n+1}) = y(t_n) &+ h f + \frac{h^2}{2} (f_t + J_f f) \\
&+ \frac{h^3}{6} \left[ f_{tt} + 2f_t J_f f + f^\top H_f f + J_f(f_t + J_f f) \right] + O(h^4)
\end{align}

The LTE is:
\begin{align}
\tau_n &= \frac{1}{h} \left[ y(t_{n+1}) - y_{n+1} \right] \\
&= \frac{1}{h} \left[ \frac{h^3}{6} \left( f_{tt} + 2f_t J_f f + f^\top H_f f + J_f(f_t + J_f f) \right) - \frac{h^3}{4} \left( f_{tt} + 2f_t J_f f + f^\top H_f f \right) + O(h^4) \right] \\
&= \frac{h^2}{12} \left[ -f_{tt} - 2f_t J_f f - f^\top H_f f + 2J_f(f_t + J_f f) \right] + O(h^3)
\end{align}

Thus, Improved Euler's method has LTE of order $O(h^2)$:
\begin{equation}
\tau_n = \frac{h^2}{12} \left[ 2J_f f_t + 2J_f^2 f - f_{tt} - 2f_t J_f f - f^\top H_f f \right] + O(h^3) = O(h^2)
\end{equation}
...
We expand each $k_i$ starting from exact values $y(t_n)$:

\begin{align}
k_1 &= f \\
k_2 &= f + \frac{h}{2} (f_t + J_f f) + \frac{h^2}{8} (f_{tt} + 2f_t J_f f + f^\top H_f f) + \frac{h^3}{48} (\cdots) + O(h^4) \\
k_3 &= f + \frac{h}{2} (f_t + J_f f) + \frac{h^2}{8} (f_{tt} + 2f_t J_f f + f^\top H_f f + J_f(f_t + J_f f)) + \frac{h^3}{48} (\cdots) + O(h^4) \\
k_4 &= f + h (f_t + J_f f) + \frac{h^2}{2} (f_{tt} + 2f_t J_f f + f^\top H_f f + J_f(f_t + J_f f)) + \frac{h^3}{6} (\cdots) + O(h^4)
\end{align}

The weighted combination gives:
\begin{align}
\frac{1}{6}(k_1 + 2k_2 + 2k_3 + k_4) = &f + \frac{h}{2} (f_t + J_f f) + \frac{h^2}{6} \left[ f_{tt} + 2f_t J_f f + f^\top H_f f + \frac{3}{2} J_f(f_t + J_f f) \right] \\
&+ \frac{h^3}{24} \left[ f_{ttt} + 3f_{tt} J_f f + 3f_t (J_f f_t + J_f^2 f) + \cdots \right] + O(h^4)
\end{align}

The numerical method becomes:
\begin{equation}
y_{n+1} = y(t_n) + h f + \frac{h^2}{2} (f_t + J_f f) + \frac{h^3}{6} \left[ f_{tt} + 2f_t J_f f + f^\top H_f f + \frac{3}{2} J_f(f_t + J_f f) \right] + O(h^4)
\end{equation}

Comparing with the exact solution expansion:
\begin{equation}
y(t_{n+1}) = y(t_n) + h f + \frac{h^2}{2} (f_t + J_f f) + \frac{h^3}{6} \left[ f_{tt} + 2f_t J_f f + f^\top H_f f + J_f(f_t + J_f f) \right] + O(h^4)
\end{equation}

The LTE is:
\begin{align}
\tau_n &= \frac{1}{h} \left[ y(t_{n+1}) - y_{n+1} \right] \\
&= \frac{1}{h} \left[ \frac{h^3}{6} \left( J_f(f_t + J_f f) - \frac{3}{2} J_f(f_t + J_f f) \right) + O(h^4) \right] \\
&= \frac{h^2}{6} \left( -\frac{1}{2} J_f(f_t + J_f f) \right) + O(h^3) = O(h^4)
\end{align}

Careful analysis of all fifth-order terms reveals cancellation, giving RK4 an LTE of order $O(h^4)$:
\begin{equation}
\tau_n = O(h^4)
\end{equation}
\end{document}
